  ...
  <!-- import menu component -->
    ...
    <div style="padding:20px; background:#E0E0E0; height: 100%;">
      ...
      <div>
        ...
        <v-row style="margin-top: -10px; margin-bottom: 30px;margin-right:15px;float: right;margin-left: 20px;">
          <!-- <v-icon @click="searchTag" size="50" color="blue" style="cursor: pointer;"> mdi-card-search</v-icon> -->
          <v-btn @click="searchTag"  style="cursor: pointer;" color="blue">Search</v-btn>
        </v-row>
      </div>
        ...
        >
          <template v-slot:item="{item}">
            <tr >
              ...
              <td >
                <div class="d-flex">
                  <div @click="EditTag(item)" style="margin-right: 5px; background: green;border-radius: 50%; width: 30px; height: 30px; align-items: center !important; display: flex; justify-content: center !important;">
                    <v-icon size="17" color="white" > mdi-pencil</v-icon>
                    <ToolTipMessage message="Edit Tag"></ToolTipMessage>
                  </div>
                  <div @click="DeleteTag(item.ID,item.name)"  style="background: red; border-radius: 50%; width: 30px; height: 30px; align-items: center !important; display: flex; justify-content: center !important;">
      ...
      <v-dialog v-model="dialogDelete" persistent max-width="500px">
        <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row >
                  <p>Are you sure, you want to delete Tag <q>{{ name  }}</q> ?</p>
                </v-row>
              </v-container>
            </v-card-text>
    ...
    data: () => ({
      ...
      methods: {
        ...
        getTag(){
          ...
          axios.get(process.env.VUE_APP_API_URL+'/tags',{params}).then(res=>{
        ...
        searchTag(){
          if(this.s_name != "" || this.userSelected != null){
            if(this.userSelected != null){
              this.username = this.userSelected == "All" ? this.userSelected : this.userSelected.username;
            }
            let params = {page:this.page, itemPerPage:this.itemsPerPage, name: this.s_name,created_by:this.username};
            axios.get(process.env.VUE_APP_API_URL+'/tags/filter',{params},{ validateStatus: () => false }).then(res=>{
              this.tagList = res.data.data.tags;
              this.totalItems = res.data.data.pagination.total_record;
              this.loading = false;  
            })
        ...
        SaveTag(){
          ...
          if(this.tag_id > 0){
            axios.put(process.env.VUE_APP_API_URL+'/tag',tag,{ validateStatus: () => true }).then((res)=>{
              this.message = res.data.message;
              this.AddUpdateData(res.data.id);
            })
          }else{
            axios.post(process.env.VUE_APP_API_URL+'/tag',tag,{ validateStatus: () => true }).then((res)=>{
              this.message = res.data.message;
              this.AddUpdateData(res.data.id);
            })
          }
        ...
        ConfirmDeleteItem(){
          axios.delete(process.env.VUE_APP_API_URL+'/tag/'+this.tag_id,{ validateStatus: () => true }).then((res)=>{
            this.message = res.data.message;
            if(res.data.id==1){
              this.backgroundColor="green";
            }else{
