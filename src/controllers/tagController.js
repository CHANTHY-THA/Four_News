...
const addTag = async (req, res) => {
    try{ 
        ...
        const foundTag = await prisma.tag.findFirst({where:{name:name}});
        if(!foundTag){
...
const updateTag = async (req,res) =>{
    try{
        ...
        const foundTag = await prisma.tag.findFirst({where:{ID:id, status:1}});
        console.log("data: " + foundTag);
        if(!foundTag){
            return new Response(res).setID(0).setStatusCode(404).setMessage("No data found.").send();
        }

        const checkRecordExist = await prisma.tag.findFirst({where:{name:name}});
